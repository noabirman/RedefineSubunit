...
if af3:
    ...
    if summary_file_path is not None and os.path.exists(summary_file_path):
        ...
        # FIX: Create a mapping based on the sorted order of chains found in the file
        # This maps the first chain (e.g. 'B') to 0, the second (e.g. 'C') to 1.
        sorted_chains = sorted(unique_chains)

        for chain1 in unique_chains:
            # Use the index from the sorted list instead of ord()
            try:
                nchain1 = sorted_chains.index(chain1)
            except ValueError:
                continue
            ...
            for chain2 in unique_chains:
                ...
                try:
                    nchain2 = sorted_chains.index(chain2)

                    # Safety check to ensure we don't crash if matrix is smaller than expected
                    if nchain1 < len(af3_chain_pair_iptm_data) and nchain2 < len(af3_chain_pair_iptm_data[nchain1]):
                        iptm_af3[chain1][chain2] = af3_chain_pair_iptm_data[nchain1][nchain2]
                    else:
                        print(f"Warning: Matrix index out of bounds for chains {chain1}-{chain2}. Setting iptm to 0.")
                        iptm_af3[chain1][chain2] = 0.0
